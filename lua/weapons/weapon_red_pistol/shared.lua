...
if CLIENT then
language.Add("weapon_gblaster_pistol", "Red Pistol")
...
SWEP.VElements = {
	["v_element"] = { type = "Model", model = "models/evangelos/undertale/gasterblaster.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(10.329, 1.468, -4.007), angle = Angle(-8.254, 0, 180), size = Vector(0.15, 0.15, 0.15), color = Color(255, 0, 0, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} },
	...
	["v_element2"] = { type = "Model", model = "models/props_lab/labpar5.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(5.901, 1.468, -4.647), angle = Angle(0, 90, 0), size = Vector(0.2, 0.6, 0.2), color = Color(255, 5, 5, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["w_element2"] = { type = "Model", model = "models/props_lab/labpar5.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(6.298, 2.348, -5.276), angle = Angle(-23.844, 90, 4.791), size = Vector(0.2, 0.4, 0.2), color = Color(255, 5, 5, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} },
	["w_element"] = { type = "Model", model = "models/evangelos/undertale/gasterblaster.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(9.213, 2.348, -5.276), angle = Angle(-23.73, 0, 174), size = Vector(0.1, 0.1, 0.1), color = Color(255, 0, 0, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:Deploy()
	...
	self.BeatSound = CreateSound( self.Owner, Sound( "weapons/gblaster/sans2_loop.wav" ) )
...
function SWEP:Think()
	...
	if( self.Owner:KeyDown( IN_ATTACK ) ) then
		self.Weapon:SetNWBool( "IsGBZapOn", true )
		HumSnd = Sound("weapons/gblaster/tc_19_brokenbeam.wav")  --BMS_ambience/machine_ambience/m_ambience_auxlaser1.wav
		self.GBZapLoop = CreateSound(self.Weapon, HumSnd )
		self.GBZapLoop:Play()
		local tr = self.Owner:GetEyeTrace()
        if tr.Hit then
			local damageinform = DamageInfo()
			damageinform:SetDamage(100)
			damageinform:SetAttacker(self.Owner)
			damageinform:SetInflictor(self)
			damageinform:SetDamageType(DMG_DISSOLVE)
			util.BlastDamageInfo(damageinform, tr.HitPos, 128)
        end
	end
	if ( self.Owner:KeyReleased ( IN_ATTACK ) ) then
		self.Weapon:EmitSound("ambient/energy/weld1.wav", 50, 100)
		self.GBZapLoop:Stop()
	end
	if ( not self.Owner:KeyDown ( IN_ATTACK ) ) then
		self.Weapon:SetNWBool( "IsGBZapOn", false )
	end
end

if CLIENT then

	local LaserBeamGB = Material( "effects/lgtning2" )
	local LaserSpriteGB = Material( "sprites/emitter_flare2" )
	local LaserSpriteGB2 = Material( "sprites/emitter_flare3" )
	...
	hook.Add( "RenderScreenspaceEffects", "GBLASTER.RenderScreenspaceEffects", function()
		for k,v in ipairs( player.GetAll() ) do
			local weap = v:GetActiveWeapon()

			if IsValid( weap ) and weap:GetNWBool( "IsGBZapOn", false ) then
				cam.Start3D( EyePos(), EyeAngles() )
					...
					if beamstartpos then
						...
						render.SetMaterial( LaserBeamGB )
						render.DrawBeam( beamstartpos, beamendpos, size, 10, 2, Color(255, 0, 0, 255)  )
						render.SetMaterial( LaserBeamGB )
						render.DrawBeam( beamstartpos, beamendpos, size, 10, 2, Color(255, 0, 0, 255)  )
						render.SetMaterial( LaserBeamGB )
						render.DrawBeam( beamstartpos, beamendpos, size, 10, 2, Color(255, 0, 0, 255)  )
						render.SetMaterial( LaserBeamGB )
						render.DrawBeam( beamstartpos, beamendpos, size, 10, 2, Color(255, 0, 0, 255)  )
						render.SetMaterial( LaserSpriteGB )
						render.DrawSprite( beamendpos, 512, 512, Color(255, 0, 0, 255)  )
						render.SetMaterial( LaserSpriteGB )
						render.DrawSprite( beamendpos, 512, 512, Color(255, 0, 0, 255)  )
						render.SetMaterial( LaserSpriteGB2 )
						render.DrawSprite( beamendpos, 512, 512, Color(255, 0, 0, 255)  )
						render.SetMaterial( LaserSpriteGB2 )
						render.DrawSprite( beamendpos, 512, 512, Color(255, 0, 0, 255)  )
					end
	...
	for _, ply in pairs ( player.GetAll() ) do
			if ( ply:GetActiveWeapon():IsValid() ) then
				if ( ply:GetActiveWeapon():GetClass() == "weapon_supreme_badtime_bm_gblaster" || ply:GetActiveWeapon():GetClass() == "weapon_supreme_badtime_bm_gblaster" ) then
					local light = DynamicLight( ply:EntIndex() )
					if ( light ) then
						light.r = 255
						light.g = 0
						light.b = 0
						light.Pos = beamendpos
						light.Brightness = 10
						light.Size = 80
						light.Decay = 2
...
function SWEP:PrimaryAttack()
	...
	if ( self.LoopSound ) then
		...
		self.LoopSound = CreateSound( self.Owner, Sound( "weapons/gblaster/badtime2_loop.mp3" ) )
	...
	local bullet = {} 
		...
		self.Owner:EmitSound("weapons/gblaster/gblaster_fire.mp3", 100, math.random( 80, 90 ))
		self.Owner:EmitSound(Sound("weapons/gauz_fire/bmgauss2.mp3"), 100, math.random( 80, 90 ), 0.3)
		--self.Owner:EmitSound(Sound("ambient/machines/thumper_hit.wav"))
		self.Owner:EmitSound(Sound("ambient/machines/thumper_dust.wav"), 100, math.random( 80, 90 ), 0.5 )
...
function SWEP:DrawHUD()
    ...
    surface.SetDrawColor(Color(255,0,10,230))
